...
var endTime = new Date(2019, 9, 7, 15, 00, 23); //月份从0开始，使用7月传的是6月，截止时间
//由于每个数字最高是十位数，所以最多倒计时显示99小时，也就是四天多，否则报错，请注意
...
function update() {
    ...
    if (nextSeconds != curSeconds) { //如果当前秒数和下一次的秒数不等，说明数字还在变化
        //对比时分秒，那个发生了变化，则对应的加上小球
        if (parseInt(curHours / 10) != parseInt(nextHours / 10)) { //小时的十分位
            addBalls(MARGIN_LEFT + 0, MARGIN_TOP, parseInt(nextHours / 10)) //找到小时十分位的位置就是left和top的位置，以及相应的十位数的数字
        }
        if (parseInt(curHours % 10) != parseInt(nextHours % 10)) { //小时的个位
            addBalls(MARGIN_LEFT + 15 * (RADIUS + 1), MARGIN_TOP, parseInt(nextHours % 10)) //找到小时十分位的位置就是left和top的位置，以及相应的十位数的数字
        }
        //同理对分钟处理
        if (parseInt(curMinutes / 10) != parseInt(nextMinutes / 10)) { //小时的十分位
            addBalls(MARGIN_LEFT + 39 * (RADIUS + 1), MARGIN_TOP, parseInt(nextMinutes / 10)) //找到小时十分位的位置就是left和top的位置，以及相应的十位数的数字
        }
        if (parseInt(curMinutes % 10) != parseInt(nextMinutes % 10)) { //小时的个位
            addBalls(MARGIN_LEFT + 54 * (RADIUS + 1), MARGIN_TOP, parseInt(nextMinutes % 10)) //找到小时十分位的位置就是left和top的位置，以及相应的十位数的数字
        }

        if (parseInt(curSeconds / 10) != parseInt(nextSeconds / 10)) { //小时的十分位
            addBalls(MARGIN_LEFT + 78 * (RADIUS + 1), MARGIN_TOP, parseInt(nextSeconds / 10)) //找到小时十分位的位置就是left和top的位置，以及相应的十位数的数字
        }
        if (parseInt(curSeconds % 10) != parseInt(nextSeconds % 10)) { //小时的个位
            addBalls(MARGIN_LEFT + 93 * (RADIUS + 1), MARGIN_TOP, parseInt(nextSeconds % 10)) //找到小时十分位的位置就是left和top的位置，以及相应的十位数的数字
        }
...
function addBalls(x, y, num) {
    for (var i = 0; i < digit[num].length; i++) { //num表示渲染成数字几，对应着digit数组中的第几个子数组
        for (var j = 0; j < digit[num][i].length; j++) { //digit[num][i] 循环的是每个子数组中的第几行
            if (digit[num][i][j] == 1) { //如果该行中有等于1的就渲染成小球
                var oneBall = {
...
function renderDigit(x, y, num, cxt) {
    ...
    for (var i = 0; i < digit[num].length; i++) { //num表示渲染成数字几，对应着digit数组中的第几个子数组
        for (var j = 0; j < digit[num][i].length; j++) { //digit[num][i] 循环的是每个子数组中的第几行
            if (digit[num][i][j] == 1) { //如果该行中有等于1的就渲染成原点
                cxt.beginPath();
